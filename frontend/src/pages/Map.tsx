...
const MapPage = () => {
  ...
  const memoizedCatList = useMemo(() => <CatListContent cats={cats} />, [cats]);
    ...
    <div className="map-root mt-16 flex flex-row w-full h-[calc(100vh-64px)] overflow-hidden bg-stone-50 dark:bg-slate-800 relative z-10">
      ...
      <aside className="hidden lg:flex flex-col w-80 border-r border-stone-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-y-auto z-20 px-5 py-7 gap-5">
        ...
        <div className="flex items-center gap-2">
          <h2 className="text-stone-700 dark:text-stone-200 font-semibold">Nearby Cats</h2>
      ...
      <main className="flex-1 relative h-full w-full z-10">
        <div className="w-full h-full relative z-0">
          ...
          >
            <img src={cat_white} className="w-6 h-6" alt="cats" />
      ...
      >
        ...
        <div
          className={`absolute bottom-0 left-0 right-0 bg-white dark:bg-slate-900 rounded-t-[40px] shadow-2xl transition-transform duration-500 cubic-bezier(0.32, 0.72, 0, 1) flex flex-col ${isDrawerOpen ? 'translate-y-0' : 'translate-y-full'}`}
          style={{ height: '75vh' }}
        >
